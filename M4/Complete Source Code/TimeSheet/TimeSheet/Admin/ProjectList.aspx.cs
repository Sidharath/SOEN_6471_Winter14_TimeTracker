...
public partial class Admin_ProjectList : System.Web.UI.Page
{
  ...
  {
    Response.Redirect(clsFunctions.GetSiteRoot() + "/admin/ProjectDetail.aspx");
  } 
  #endregion
  ...
  {
    DataSet loDataSet = loProject.Load(txtSearch.Text.Trim());
    if (loDataSet != null)
    {
  ...
  protected void gvPlanList_RowDeleting(object sender, GridViewDeleteEventArgs e)
  {
    ...
    loProject.nmProjectID = clsFunctions.ToInteger(lsProjectID);

    if (string.IsNullOrEmpty(loProject.Delete()))
    {
  ...
  public void fillGrid()
  {
    ...
    try
    {
      dsValue = loProject.Load(txtSearch.Text.Trim());
