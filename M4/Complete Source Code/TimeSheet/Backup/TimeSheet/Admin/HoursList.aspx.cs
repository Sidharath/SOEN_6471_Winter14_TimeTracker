...
public partial class Admin_HoursList : System.Web.UI.Page
{
  Hours loHours = new Hours();
  ...
  {
    Response.Redirect(clsFunctions.GetSiteRoot() + "/admin/HoursDetail.aspx");
  } 
  #endregion
  ...
  {
    DataSet loDataSet = loHours.Load(txtSearch.Text.Trim());
    if (loDataSet != null)
    {
  ...
  protected void gvPlanList_RowDeleting(object sender, GridViewDeleteEventArgs e)
  {
    ...
    loHours.nmHourID = clsFunctions.ToInteger(lsProjectID);

    if (string.IsNullOrEmpty(loHours.Delete()))
    {
    ...
    {
      Utility.getMessageFormat(lblMessage, Message.TIME_DELETE, true);
    }
  ...
  public void fillGrid()
  {
    ...
    try
    {
      dsValue = loHours.Load(txtSearch.Text.Trim());
